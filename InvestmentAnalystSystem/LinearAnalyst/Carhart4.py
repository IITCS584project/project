...
def Main2():
    ...
    #due_date = 20190415
    reader = read_data()
    succ,info,factor_data = reader.get_factor_daily(start_date=start_date,end_date=due_date,columns=['trade_date', 'mkt_rf','smb','hml', 'umd', 'rf'])
    ...
    umd = factor_data[:,4] * 100
    umd = umd.reshape(len(umd),1)
    rf = factor_data[:, 5] * 100
    ...
    X = np.concatenate([mkt_rf, smb, hml, umd], axis=1)
    y = stock_yield - rf    
    y = y.reshape(len(y),1)
